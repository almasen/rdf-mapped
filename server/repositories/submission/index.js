...
const insert = (submission) => {
    const query = "INSERT INTO submission(id, status, submitter, data) VALUES ($1, $2, $3, $4) " +
        "RETURNING *"; // returns passed submission with it's id set to corresponding id in database
    const params = [submission.id, submission.status, submission.submitter, submission.data];
...
const update = (submission) => {
    const query = "UPDATE submission SET status = $2, submitter = $3, data = $4 WHERE id = $1" +
        "RETURNING *"; // returns passed submission with it's id set to corresponding id in database
    const params = [submission.id, submission.status, submission.submitter, submission.data];
    return db.query(query, params);
};
