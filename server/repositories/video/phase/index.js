...
const findAllByVideoId = (videoId) => {
    const query = "SELECT * FROM video_phase WHERE video_id=$1";
    return db.query(query, [videoId]);
};

const findAllByPhaseId = (phaseId) => {
    const query = "SELECT * FROM video_phase WHERE phase_id=$1";
    return db.query(query, [phaseId]);
};

const find = (videoId, phaseId) => {
    const query = "SELECT * FROM video_phase WHERE video_id=$1 AND phase_id=$2";
    const params = [videoId, phaseId];
    return db.query(query, params);
};

const removeByVideoId = (videoId) => {
    const query = "DELETE FROM video_phase WHERE video_id = $1 RETURNING *";
    return db.query(query, [videoId]);
};
